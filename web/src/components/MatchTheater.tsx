        ...
        >
          ...
          <div className="hidden sm:block" aria-hidden>
            {markets.slice(0, 7).map((m, i) => (
              <MarketChip key={m.id} market={m} label={CHIP_LABEL[i] ?? m.spec.title} x={8 + i * 14} />
            ))}
          </div>
...
/** One market as a small standing billboard on the far side of the pitch. */
function MarketChip({ market, label, x }: { market: MarketState; label: string; x: number }) {
  ...
  return (
    <div className="theater-anchor" style={{ left: `${x}%`, top: "-5%" }}>
      ...
      >
        ...
        <span className={`block font-mono ${resolved ? "text-proof-300" : "text-slate-400"}`}>
